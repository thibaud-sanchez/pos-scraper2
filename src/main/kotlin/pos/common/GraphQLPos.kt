...
abstract class GraphQLPos(posName: String) : BasePos(posName) {
    protected var NB_PARRALLEL_THREADS = 1
...
//        "https://tonyvoyce.com/",
    ...
    var restaurantRequestQueue: Queue<RestaurantGraphQLRequest> = LinkedList<RestaurantGraphQLRequest>()
    override fun start() {
    ...
    private fun saveFailedRestaurantResponse(
        request: RestaurantGraphQLRequest,
        status: RestaurantStatus,
        responseBody: String?,
        extra: String
    ) {
        database.saveRestaurant(request.id, status, request.query, responseBody, extra)
    ...
    @Synchronized
    private fun getNextRestaurantRequest(): RestaurantGraphQLRequest? {
    ...
    abstract fun determineResponseStatus(request: RestaurantGraphQLRequest, responseBody: String?): RestaurantStatus
