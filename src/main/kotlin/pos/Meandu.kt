...
class Meandu : HttpPos("meandu") {

    val regex = listOf<RestaurantIdRegex>(
        RestaurantIdRegex(
            "slug",
            """https?:\/\/(www\.)?meandu\.app\/(?<id>[a-zA-Z\-0-9]*)""".toRegex()
        ),
    )
    ...
    private fun generateMrYumRequest(slug: String): RestaurantHttpRequest {
        ...
        val request = HttpPost("https://ap1-guest-gateway.meandu.app/graphql")
    ...
    override fun determineResponseStatus(
        ...
        return if (json.has("data") && json.getJSONObject("data").optJSONObject("guestVenue")!=null) {
            totalcurrent++
    ...
    private fun JSONObject.toRestaurant(restaurantDb: RestaurantDb): Restaurant {
        ...
        return Restaurant(
            ...
            contactEmail = json.optString("supportEmail" ),
            contactPhone = json.optString("supportPhone" ),
