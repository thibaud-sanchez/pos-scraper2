...
class Zelty : HttpPos("zelty") {
    private val ahrefsDomains=listOf("order.zelty.fr","booking.zelty.fr")

    val regex = listOf<RestaurantIdRegex>(
        RestaurantIdRegex("order", """\"https?:\/\/order\.zelty\.fr\/(.*?)zkey(=|-)(?<id>.*?)(\"|&)""".toRegex()),
        RestaurantIdRegex("order", """https?:\/\/order\.zelty\.fr\/(.*?)zkey(=|-)(?<id>.*?)(\"|&|,|;)""".toRegex()),
        RestaurantIdRegex("order", "window._zwg.key.*?'(?<id>.*?)'".toRegex()),
        RestaurantIdRegex("booking", """\"https?:\/\/booking\.zelty\.fr\/(.*?)zkey(=|-)(?<id>.*?)(\"|&)""".toRegex()),
    )
    ...
    fun resolveGoogleids() {
        resolverGoogle.parseGoogleResult("site:order.zelty.fr",false)
    }
